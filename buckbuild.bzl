...
QNNPACK_DEP = "//aten/src/ATen/native/quantized/cpu/qnnpack:pytorch_qnnpack" if is_oss_build() else "//xplat/caffe2/aten/src/ATen/native/quantized/cpu/qnnpack:pytorch_qnnpack"
...
def define_buck_targets(
    ...
    [
        fb_xplat_cxx_library(
            name = "aten_native_cpu" if is_native else "aten_cpu",
            srcs = aten_native_source_list if is_native else ([
                # Generated
            ...
            ] + jit_core_sources + aten_cpu_source_list),
            linker_flags = ["-Wl,--no-as-needed", "-ldl"] if is_oss_build() else [],
            header_namespace = "",
            link_whole = True,
            visibility = ["PUBLIC"],
            deps = [
                get_third_party_lib("cpuinfo"),
                get_third_party_lib("glog"),
                get_third_party_lib("XNNPACK"),
            ] + ([] if is_oss_build() else [
                "//xplat/third-party/linker_lib:omp",
            ]),
            exported_deps = [
                get_third_party_lib("fmt"),
                get_third_party_lib("ruy"),
                QNNPACK_DEP,
                C10_DEP,
                ":aten_header",
                ":caffe2_headers",
